...
// Update the RootStackParamList to include isFlexible
declare global {
...
const DailyPlanFrequency = () => {
  ...
  const handleNext = () => {
    ...
    navigation.navigate('DailyPlanSchedule', {
      ...
      selectedDays: selectedFrequency === 'specific-days-week' 
        ? daysOfWeek.filter(day => day.selected).map(day => day.name)
        : monthDays.filter(day => day.selected).map(day => day.day.toString()),
      isFlexible,
    });
  };
